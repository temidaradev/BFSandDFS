...
func (fd *FileDialog) Draw(screen *ebiten.Image) {
	...
	// Draw title
	text.Draw(screen, fd.SaveLabel, basicfont.Face7x13, fd.X+10, fd.Y+20, color.Black)
	...
	text.Draw(screen, dirText, basicfont.Face7x13, fd.X+10, fd.Y+40, color.RGBA{50, 50, 150, 255})

	// Draw separator
	separator := ebiten.NewImage(fd.Width-20, 1)
	separator.Fill(color.RGBA{180, 180, 180, 255})
	opts = &ebiten.DrawImageOptions{}
	...
	for i := fd.ScrollOffset; i < endIdx; i++ {
		...
		if i == fd.SelectedFile {
			...
			selectionBg.Fill(color.RGBA{180, 200, 255, 255})
			opts = &ebiten.DrawImageOptions{}
		...
		// Draw file name
		fileName := fd.Files[i]
		fileColor := color.RGBA{0, 0, 0, 255} // Black
		if strings.HasSuffix(fileName, "/") {
			fileColor = color.RGBA{0, 0, 150, 255}
		}
	...
	if fd.IsSaveDialog {
		text.Draw(screen, "Filename:", basicfont.Face7x13, fd.X+10, fd.Y+fd.Height-60, color.Black)
		
		// Input field background
		inputBg := ebiten.NewImage(fd.Width-20, 25)
		inputBg.Fill(color.RGBA{255, 255, 255, 255})
		opts = &ebiten.DrawImageOptions{}
		opts.GeoM.Translate(float64(fd.X+10), float64(fd.Y+fd.Height-55))
		screen.DrawImage(inputBg, opts)
		
		// Draw input field border
		inputBorderColor := color.RGBA{150, 150, 150, 255}
		...
		// Draw filename text
		text.Draw(screen, fd.FileName, basicfont.Face7x13, fd.X+15, fd.Y+fd.Height-38, color.Black)
		
		// Draw cursor
		cursorPos := text.BoundString(basicfont.Face7x13, fd.FileName[:fd.CursorPos]).Dx()
		cursorHeight := 18
		for i := 0; i < cursorHeight; i++ {
			screen.Set(fd.X+15+cursorPos, fd.Y+fd.Height-48+i, color.RGBA{0, 0, 0, 255})
		}
	}
